...
export async function getGerencia(req: Request, res: Response) {
  try {
    ...
    const tratamentoStats = {
      ...
      acontecem: {
        psicoterapia: pacientes.filter(p =>
          p.tipoDeTratamento?.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "") === "psicoterapia"
        ).length,
        plantao: pacientes.filter(p =>
          p.tipoDeTratamento?.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "") === "plantao"
        ).length,
        psicodiagnostico: pacientes.filter(p =>
          p.tipoDeTratamento?.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "") === "psicodiagnostico"
        ).length,
        avaliacao_diagnostica: pacientes.filter(p =>
          p.tipoDeTratamento?.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "") === "avaliacao diagnostica"
        ).length,
      }
